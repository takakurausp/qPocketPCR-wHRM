...
WiFi 搭載ビルド（`WIFI_ENABLED=1`、ビルド時は `-DWIFI_ENABLED=1`）では、起動時に以下のいずれかで Web 制御用ネットワークを形成します。どちらの振る舞いになるかは USB ドライブ上の `WIFI.TXT` の内容で決まります。
...
> 日本国内でWiFiを使用する場合、またはRF発射を避ける場合は、ビルド時にWiFiを無効にして書き込んでください。>### WiFi を無効にしてビルドする
...
本ファirmwareは、原版の[qPocketPCR](https://github.com/GaudiLabs/qPocketPCR)（GaudiLabs、Urs Gaudenz）をベースにした独立したフォーク **qPocketPCR-wHRM v0.24** です。原版はそのまま **V1.1** を維持し、本フォークで高分解能融解曲線（HRM）・128 KB USB Mass Storage・WiFi によるWeb制御・名前付きプロトコル保存を追加しました。
